    ...
    let api = {
        ...
        handler: function (req, res) {
            ...
            try {
                ...
                //width = 700, height = 400, scale = 3, highchartsOptions
                let width = get(data, 'width', null)
                let height = get(data, 'height', null)
                let scale = get(data, 'scale', null)
                let opt = get(data, 'opt', '')
                if (isestr(opt)) {
                    opt = json.parse(opt) //傳入是string得轉回object
                }
                // console.log('opt', opt)

                //wh
                wh(width, height, scale, opt)
                    .then((b64) => {
                    // console.log('then', b64)
                        b64 = `data:image/png;base64, ${b64}`
                        // b64 = `<img src="${b64}">`
                        // r = b64
                        r = JSON.stringify({ b64 })
                    })
                    .catch((err) => {
                ...
                r = JSON.stringify({ err: err.toString() })
                pm.resolve(r)
            }
